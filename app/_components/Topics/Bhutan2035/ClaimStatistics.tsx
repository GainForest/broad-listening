...
const calculateAllTheAgeGroups = (demographics: TDemographics, topic: TTopic) => {
    const people = new Set<string>();
    topic.subtopics.forEach((subtopic) => {
        subtopic.claims.forEach((claim) => {
            claim.quotes.forEach((quote) => {
    ...
    const under18 = ageData.under_18?.filter((id: string) => authorIds.includes(id)).length || 0;
    const age18_25 = ageData.age_18_25?.filter((id: string) => authorIds.includes(id)).length || 0;
    const age25_35 = ageData.age_25_35?.filter((id: string) => authorIds.includes(id)).length || 0;
    const age35_55 = ageData.age_35_55?.filter((id: string) => authorIds.includes(id)).length || 0;
    const over55 = ageData.over_55?.filter((id: string) => authorIds.includes(id)).length || 0;
